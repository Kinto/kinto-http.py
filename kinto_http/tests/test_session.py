...
class SessionTest(unittest.TestCase):
    ...
    def test_no_auth_is_used_by_default(self):
        ...
        self.requests_mock.request.assert_called_with(
            "get", "https://example.org/test", headers=self.requests_mock.request.headers
        )
    ...
    def test_session_injects_auth_on_requests(self):
        response = fake_response(200)
        self.requests_mock.request.return_value = response
        session = Session(auth=mock.sentinel.auth, server_url="https://example.org")
        session.request("get", "/test")
        self.requests_mock.request.assert_called_with(
            "get",
    ...
    def test_url_is_used_if_schema_is_present(self):
        ...
        self.requests_mock.request.assert_called_with(
            "get", "https://example.org/anothertest", headers=self.requests_mock.request.headers
        )

    def test_creation_fails_if_session_and_server_url(self):
        self.assertRaises(
            AttributeError, create_session, session="test", server_url="http://example.org"
        )
        self.assertRaises(
            AttributeError, create_session, "test", session="test", auth=("alexis", "p4ssw0rd")
        )
    ...
    def test_creates_a_session_if_needed(self, session_mock):
        ...
        create_session(server_url=mock.sentinel.server_url, auth=mock.sentinel.auth)
        session_mock.assert_called_with(
            server_url=mock.sentinel.server_url, auth=mock.sentinel.auth
        )
...
class RetryRequestTest(unittest.TestCase):
    ...
    def test_retries_on_409_errors(self):
        self.requests_mock.request.side_effect = [self.response_409, self.response_200]
        session = Session("https://example.org", retry=1)
        session.request("GET", "/v1/foobar")  # Not raising.

    def test_does_not_wait_if_retry_after_header_is_not_present(self):
        self.requests_mock.request.side_effect = [self.response_503, self.response_200]
