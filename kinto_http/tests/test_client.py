...
class ClientTest(unittest.TestCase):
    ...
    def test_context_manager_works_as_expected(self):
        ...
        self.session.request.side_effect = [({"settings": settings}, []), ({"responses": []}, [])]
    ...
    def test_client_is_represented_properly_with_bucket_and_collection(self):
        client = Client(
            server_url="https://kinto.notmyidea.org/v1", bucket="homebrewing", collection="recipes"
        )
        expected_repr = (
            "<KintoClient https://kinto.notmyidea.org/v1/"
    ...
    def test_client_is_represented_properly_with_bucket(self):
        client = Client(server_url="https://kinto.notmyidea.org/v1", bucket="homebrewing")
        expected_repr = "<KintoClient https://kinto.notmyidea.org/v1/" "buckets/homebrewing>"
        assert str(client) == expected_repr

    def test_client_is_represented_properly_without_bucket(self):
        client = Client(server_url="https://kinto.notmyidea.org/v1", bucket=None)
...
class BucketTest(unittest.TestCase):
    ...
    def test_update_bucket_handles_if_match(self):
        self.client.update_bucket(id="testbucket", data={"foo": "bar"}, if_match=1234)
        self.session.request.assert_called_with(
            "put",
            "/buckets/testbucket",
            data={"foo": "bar"},
    ...
    def test_get_is_issued_on_list_retrieval(self):
        ...
        self.session.request.assert_called_with("get", "/buckets", headers={}, params={})
    ...
    def test_delete_is_issued_on_list_deletion(self):
        ...
        self.session.request.assert_called_with("delete", "/buckets", headers=None)

    def test_get_or_create_dont_raise_in_case_of_conflict(self):
        bucket_data = {"permissions": mock.sentinel.permissions, "data": {"foo": "bar"}}
        self.session.request.side_effect = [get_http_error(status=412), (bucket_data, None)]
        # Should not raise.
...
class CollectionTest(unittest.TestCase):
    ...
    def test_collection_creation_issues_an_http_put(self):
        self.client.create_collection(id="mycollection", permissions=mock.sentinel.permissions)
    ...
    def test_data_can_be_sent_on_creation(self):
        self.client.create_collection(id="mycollection", bucket="testbucket", data={"foo": "bar"})
    ...
    def test_collection_update_issues_an_http_put(self):
        self.client.update_collection(
            id="mycollection", data={"foo": "bar"}, permissions=mock.sentinel.permissions
        )
    ...
    def test_update_handles_if_match(self):
        self.client.update_collection(id="mycollection", data={"foo": "bar"}, if_match=1234)
    ...
    def test_collection_update_use_an_if_match_header(self):
        data = {"foo": "bar", "last_modified": "1234"}
        self.client.update_collection(
            id="mycollection", data=data, permissions=mock.sentinel.permissions
        )
    ...
    def test_patch_collection_issues_an_http_patch(self):
        self.client.patch_collection(id="mycollection", data={"key": "secret"})
    ...
    def test_patch_collection_handles_if_match(self):
        self.client.patch_collection(id="mycollection", data={"key": "secret"}, if_match=1234)
    ...
    def test_collection_delete_if_match_not_included_if_not_safe(self):
        data = {}
        mock_response(self.session, data=data)
        deleted = self.client.delete_collection(id="mycollection", if_match=1324, safe=False)
        assert deleted == data
        url = "/buckets/mybucket/collections/mycollection"
        self.session.request.assert_called_with("delete", url, headers=None)
    ...
    def test_get_or_create_raise_in_other_cases(self):
        self.session.request.side_effect = get_http_error(status=500)
        with self.assertRaises(KintoException):
            self.client.create_collection(bucket="buck", id="coll", if_not_exists=True)
    ...
    def test_create_collection_raises_a_special_error_on_403(self):
        ...
        with self.assertRaises(KintoException) as e:
            self.client.create_collection(bucket="buck", id="coll")
        expected_msg = (
...
class RecordTest(unittest.TestCase):
    ...
    def test_records_handles_permissions(self):
        ...
        self.client.create_record(
            data={"id": "1234", "foo": "bar"}, permissions=mock.sentinel.permissions
        )
        self.session.request.assert_called_with(
            "put",
    ...
    def test_records_timestamp_is_cached(self):
        mock_response(
            self.session, data=[{"id": "foo"}, {"id": "bar"}], headers={"ETag": '"12345"'}
        )
    ...
    def test_records_timestamp_is_cached_per_collection(self):
        ...
        mock_response(
            self.session, data=[{"id": "foo"}, {"id": "bar"}], headers={"ETag": '"67890"'}
        )
    ...
    def test_pagination_supports_if_none_match(self):
        ...
        self.client.get_records(bucket="bucket", collection="collection", if_none_match="1234")
    ...
    def test_pagination_generator_if_none_match(self):
        ...
        for index, page_records in enumerate(
            self.client.get_paginated_records(if_none_match="1234")
        ):
            # Check that the If-None-Match header is present in the requests.
            assert response[index] == page_records
    ...
    def test_create_record_raises_a_special_error_on_403(self):
        self.session.request.side_effect = get_http_error(status=403)
        with self.assertRaises(KintoException) as e:
            self.client.create_record(bucket="buck", collection="coll", data={"foo": "bar"})
        expected_msg = (
